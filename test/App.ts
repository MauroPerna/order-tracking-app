...
describe("App", async function () {
  ...
  beforeEach(async () => {
    ...
    const dispatchedOrdersQueueContractAddress =
      contract.dispatchedOrdersQueue();

    pendingOrdersQueueContract = queue.attach(
      pendingOrdersQueueContractAddress
    );
    dispatchedOrdersQueueContract = queue.attach(
      dispatchedOrdersQueueContractAddress
    );
  });
  ...
  describe("Initialized", async () => {
    it("The App was deployed", async () => {
      ...
      expect(pendingOrdersQueueContract.address).to.not.equal(
        ethers.constants.AddressZero
      );
      expect(dispatchedOrdersQueueContract.address).to.not.equal(
        ethers.constants.AddressZero
      );
    });
  });
  ...
  describe("Queue", async () => {
    it("Should enqueue an item to the queue", async () => {
      ...
      await testQueueContract.testEnqueue(
        ethers.utils.formatBytes32String("random-value")
      );
  ...
  describe("Order Flow", async () => {
    ...
    beforeEach(async () => {
      ...
      await appWithOwnerAsSigner.createDispatcherWorker(
        charlie.address,
        "Charlie"
      );
      ...
      const orderCreatedPromise = new Promise(async (resolve, reject) => {
        appWithAliceAsSigner.on(
          "OrderCreated",
          (client: string, orderId: string) => {
            resolve({ client, orderId });
          }
        );

        const tx = await appWithAliceAsSigner.createOrder(quantityItemsBySKU, {
    ...
    describe("Create Order", async () => {
      it("As client, should create and order with PENDING status", async () => {
        ...
        const products = await appWithAliceAsSigner.getProductsByClient(
          orderId
        );
      ...
      it("As a customer, should be able to CANCEL an order if its status is PENDING", async () => {
        ...
        await expect(appWithAliceAsSigner.cancelOrder(orderId)).to.emit(contract, "OrderCanceled")
      ...
      it("As a warehouser, should ignore CANCELED orders", async () => {
        ...
        expect(
          appWithBobAsSigner.addOrderToPreparationStage()
        ).to.be.revertedWith("No valid order found in client queue");
      });
    });
    ...
    describe("Order In Preparation", async () => {
      it("As warehouse worker, should can add order in preparation stage and mark an order as IN_PREPARATION", async () => {
        await expect(appWithBobAsSigner.addOrderToPreparationStage()).to.emit(contract, "OrderInPreration")
    ...
    describe("Order In Deliver Stage", async () => {
      it("As warehouse worker, should can add order in deliver stage and mark an order as PREPARED", async () => {
        ...
        await expect(appWithBobAsSigner.moveOrderToDeliverStage()).to.emit(contract, "OrderPrepared")
        ...
        const existInDispatchedOrderQueue =
          await dispatchedOrdersQueueContract.containsOrderId(orderT2.orderId);
      ...
      it("As dispatcher worker, should can dispatch an order and mark an order as IN_TRANSIT", async () => {
        ...
        await expect(appWithCharlieAsSigner.dispatchOrder()).to.emit(contract, "OrderInTransit");
      ...
      it("As dispatcher worker, should can deliver an order and mark  an order as DELIVERED", async () => {
        ...
        const orderDeliverPromise = new Promise(async (resolve, reject) => {
          contract.on("OrderDelivered", (orderId: string) => {
            resolve({ orderId });
          });

          await expect(appWithCharlieAsSigner.deliverOrder()).to.emit(contract, "OrderDelivered");
        });
    ...
    describe("Order In Verification Stage", async () => {
      it("As a client, should be able to mark an order as VERIFIED", async () => {
        ...
        await expect(appWithAliceAsSigner.verifyOrder(orderId)).to.emit(contract, "OrderVerified");
      ...
      it("As a client, should be able to mark an order as received with discrepancy", async () => {
        ...
        await expect(appWithAliceAsSigner.markOrderAsReceivedWithDiscrepancy(orderId, OrderVerification.ERROR_IN_ORDER)).to.emit(contract, "OrderReceivedWithDiscrepancy");
